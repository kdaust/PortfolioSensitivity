## portfolio functions from ccissdev package to play with
cleanData_kd <- function(SSPredAll,SIBEC,SuitTable,SNum,Trees,timePer,selectBGC,SI_Class){
  ...
  SS.sum[,MeanSI := round(MeanSI/SI_Class)*SI_Class]
  SS.sum <- unique(SS.sum)
  setorder(SS.sum,Spp,FuturePeriod)
  return(SS.sum)
}

edatopicSubset_kd <- function(SSPredOrig, eda, pos = "Zonal"){
  if(pos == "Zonal"){
...
dbGetClimSum_kd <- function(con,BGC,Scn){
...
simulateClimate_kd <- function(climInfo){ ##package function
  climParams <- list()
...
dbGetSppLimits_kd <- function(con,SuitTable,Trees){
